...
// Countries that must meet transit requirements (your provided list)
const transitRequirementCountries = [
    'AF', 'AO', 'DZ', 'AZ', 'BD', 'BJ', 'BF', 'BI', 'CV', 'CM',
    'TD', 'CG', 'CD', 'CI', 'DJ', 'EG', 'GQ', 'ER', 'ET', 'GA',
    'GM', 'GE', 'GH', 'GN', 'GW', 'KZ', 'KE', 'KG', 'LR', 'ML',
    'MR', 'MZ', 'NP', 'NE', 'NG', 'PK', 'SN', 'SL', 'SO', 'LK',
    'SD', 'TJ', 'TG', 'TM', 'UZ', 'IN' // Added India
];
...
function getTouristVisaResult(nationality, countryName) {
    let visaType, visaClass, visaDescription;
    
    if (authorizedVisaCountries.includes(nationality)) {
        visaType = 'AUTHORIZED TOURIST VISA';
        visaClass = 'authorized';
        visaDescription = 'Authorized Tourist Visa';
    } else if (stampedVisaCountries.includes(nationality)) {
        visaType = 'STAMPED TOURIST VISA';
        visaClass = 'stamped';
        visaDescription = 'Stamped Tourist Visa';
    } else {
        visaType = 'TOURIST VISA';
        visaClass = 'transit';
        visaDescription = 'Tourist Visa';
    }
    ...
    return {
        ...
        message: `
            ...
            <p>As a national of ${countryName} planning to stay more than 12 hours, you need a ${visaDescription.toLowerCase()}.</p>
            <div class="action-buttons">
...
// Get result for transit
function getTransitResult(nationality, countryName) {
    if (transitRequirementCountries.includes(nationality)) {
        return {
            class: 'transit-exemptions',
            message: `
                <h3>Transit Requirements</h3>
                <div class="visa-type transit">TRANSIT REQUIREMENTS APPLY</div>
                <p>As a national of ${countryName}, you must meet at least one of the transit requirements to transit through Panama for 12 hours or less.</p>
                <div class="action-buttons">
                    <a href="transit-visa" class="requirements-btn">View Transit Requirements</a>
                </div>
            `
        };
    } else {
        // Not on transit requirement list - they need a tourist visa for transit
        return getTouristVisaForTransit(nationality, countryName);
    }
}

// Get result for countries that need tourist visa for transit
function getTouristVisaForTransit(nationality, countryName) {
    let visaType, visaClass, visaDescription;
    ...
    return {
        class: 'visa-required',
        message: `
            <h3>Tourist Visa Required for Transit</h3>
            <div class="visa-type ${visaClass}">${visaType} REQUIRED</div>
            <p>As a national of ${countryName} planning to transit through Panama for 12 hours or less, you need a ${visaDescription.toLowerCase()}.</p>
            <div class="action-buttons">
                <a href="tourist-visa" class="requirements-btn">View Required Documents</a>
            </div>
        `
